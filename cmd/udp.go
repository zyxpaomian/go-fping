...
func udpAlive(ip string) (bool, error) {
	udpTimeOut := time.Duration(timeout)
    conn, err := net.DialTimeout("udp", ip, time.Duration(udpTimeOut * time.Millisecond))
	fmt.Println(err)
    if err != nil {
		return false, err
    }
